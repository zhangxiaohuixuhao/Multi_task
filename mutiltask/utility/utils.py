...
def rc_feature_extraction(data, device_data, device_tested_number, num_pulse, padding=False):
    ...
    for i in range(img_width):
        ...
        ind = [num * 2 ** (3 - idx) for idx, num in enumerate(data[:, i].numpy())]
        ind = int(np.sum(ind))
...
def batch_rc_feat_extract_in_construction(data,
    ...
    data_random_seq = np.random.randint(0, 1, data_seq.shape)
...
def parse_args():
    ...
    '''DEVICE FILE'''
    parser.add_argument('--device_file', type=str, default='data_cell3', help='device file')

    parser.add_argument('--device', type=str, default='cpu', choices=['cpu', 'cuda:0', 'cuda:1'], help='cuda device')

    parser.add_argument('--num_pulse', type=int, default=4, help='the number of pulse in one sequence. (For '
