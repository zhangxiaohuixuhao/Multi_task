...
def train_with_feature(num_data, num_te_data,
    ...
    for epoch in range(num_epoch):
        ...
        if te_acc == max(test_acc_list):
            np.save(os.path.join(save_dir_name, f'{dataset}_test_results.npy'), conf_mat)
            # save readout layer
        ...
        for e in range(epoch):
            for (data, ddata, ldata, target, dtarget, ltarget) in test_loader:
                ...
                if target in [0, 1, 2]:
                    ...
                    te_batch_acc_total = torch.sum((te_logic.argmax(dim=-1) == target) * (te_logic2.argmax(dim=-1) == ltarget)) / batchsize
